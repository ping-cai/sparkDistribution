...
//        int k = 5;
        ...
        kspUtil.setEdges(edges);//将所有边集传入
        kspUtil.setGraph(graph);//将所有边集传入图中
...
//        int k = 5;
    ...
    private void dynamicPath(Map<String, List<DirectedPath>> odsPaths, String o, String d) {
        DirectedPath directedPath = new DirectedPath();
        Edge edge = new Edge();
        DirectedEdge directedEdge = new DirectedEdge();
        edge.setFromNode(o);
    ...
    /**
     * 通过判断od的最短一条路径来确定换乘站点数，以换乘站点数作为odk路径搜索的k
     *
...
//                            //
//                        } else {
//                            String toNodeLine = lineMap.get(directedEdges.getLast().getEdge().getToNode());
//                            String fromNodeLine = lineMap.get(directedEdge.getEdge().getToNode());
//                            if (!toNodeLine.equals(fromNodeLine)) {
...
//                                changeEdge.setToNode(directedEdge.getEdge().getToNode());
...
//                        }
            ...
            while (directedEdges.size() > 0 && ("o".equals(directedEdges.getFirst().getDirection()) || "o".equals(directedEdges.getLast().getDirection()))) {
                if ("o".equals(directedEdges.getFirst().getDirection())) {
                    directedEdges.removeFirst();
                }
                if (directedEdges.size() > 0 && "o".equals(directedEdges.getLast().getDirection())) {
                    directedEdges.removeLast();
                }
            }
            if (directedEdges.size() == 0) {
                continue;
